...
if __name__ == '__main__':
    ...
    #   Variables du script
    #######################
    model_dir = '../service_prediction/src/models/siret/siret_bi_2021-01-19/'
    ...
    EMB_TABLE = "sirus_projections_2020_manquants_nolibelle"
    ...
    #sql_sirus=f"SELECT *, 0 as blank FROM  sirus_2020 s LEFT JOIN modalites m ON s.apet = m.code where m.nct_id in (1,2,3,4,5)"
    sql_sirus=f"SELECT distinct on (sirus_id||nic) *, 0 as blank FROM  sirus_2020_noproj s LEFT JOIN modalites m ON s.apet = m.code" 
    # load model
...
#     my_driver.delete_table("sirus_projection")
    ...
    logging.info(f"Requête sirus :  {sql_sirus}")
