...
from run_top_k import load_model_from_save_dir, get_top_k
...
if __name__ == "__main__":
    ...
    ##########################
    input_dbtable_name = ['rp_quali_2018', 'sirus_2018']  # table principale

    # IMPORTANT : la table a précédence sur le save_dir
    # pour utiliser save_dir, il faut projections_dbtable_name=None
    projections_dbtable_name = 'naf_projections_quali_2018'  # table de résultats de project_dbtable.py
    save_dir = "trainings/PCS/2020-11-23_5"  # directory du modèle entrainé
    projections_dbtable_type = "naf"   # nom de la nomenclature. "naf"|"pcs" (chargé auto si on charge un modèle)
    ...
    def get_sql_request(limit=None):
        ...
        """
        req = f"SELECT {input_dbtable_name[0]}.cabbi, rs_x, actet_x, profs_x, profi_x, profa_x, siret_dec, apet, actet_c, actet_c_c, i_mca_c, i_actet_c"
        if projections_dbtable_name is not None:
             req += f", {projections_dbtable_name}.*"
        req += f" FROM {input_dbtable_name[0]}, {input_dbtable_name[1]}"
        if projections_dbtable_name is not None:
             req += f", {projections_dbtable_name}"
        req += f" WHERE (({input_dbtable_name[1]}.sirus_id = SUBSTRING({input_dbtable_name[0]}.siret_dec from 1 for 9)) AND ({input_dbtable_name[1]}.nic = SUBSTRING({input_dbtable_name[0]}.siret_dec from 10 for 5)))"
        if projections_dbtable_name is not None:
             req += f" AND ({input_dbtable_name[0]}.cabbi = {projections_dbtable_name}.cabbi)"
    ...
    def is_previously_coded(df_values):
        ...
        if nomenclature_name == "naf":
            ...
            coded['previous_auto_coded'] = df_values['actet_c_c'] == df_values['actet_c']
            ...
            coded['previous_manual_coded'] = ~coded['previous_auto_coded'] & df_values['actet_c'].notna()
            coded['previous_not_coded'] = df_values['actet_c'].isna()
            
            coded['previous_auto_validated'] = coded['previous_auto_coded'] & (df_values['apet'] == df_values['actet_c_c'])
            coded['previous_manual_validated'] = coded['previous_manual_coded'] & (df_values['apet'] == df_values['actet_c'])
            coded['previous_not_validated'] = df_values['apet'] != df_values['actet_c']
    ...
    def calculate_projection(nb_psql_docs=None):
        ...
        sql_count_request = f"SELECT COUNT(*) FROM {input_dbtable_name[0]}"
