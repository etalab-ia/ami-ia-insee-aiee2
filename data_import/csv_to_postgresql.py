    ...
    for col in dtypes.keys():
        try:
            if col.lower() not in df:
                df[col.lower()] = np.nan
            df[col.lower()] = df[col.lower()].astype(dtypes[col])
...
def create_table_description(table_name, df_table_desc):
    ...
    dictionnary = pd.Series(df_table_desc.PANDAS_TYPE.values,index=df_table_desc.FIELD_NAME).to_dict()
...
def import_csv_to_postegrsql(table_name, create_table_query, 
   ...
   # my_driver = bdd.PostGre_SQL_DB(logger=logger)   # errors already in the logs
    chunk_size = 3000 
    try:
        ...
        for df in tqdm(pd.read_csv(fichier,sep=separator_csv,chunksize=chunk_size,dtype=dtype_cols, engine='python',encoding='utf-8')):
            ...
            df = df.fillna('')
            df = df[map(str.lower, list(dtype_cols.keys())) ]
            buffer = io.StringIO()
