...
def import_csv_to_postegrsql(table_name, separator_csv, separator_buffer, fichier, dtype_cols):
    """
...
def create_table(table_name, create_table_query, force_import=False):
    """
    Importe un csv vers la base en créant/écrasant la table :param table_name:.
    ...
    try:
        ...
        logger.error(F'create_table : {e.__class__.__name__} - {e}')
        raise e
    return None
...
if __name__ == "__main__":
    ...
    for file_desc in configs['files']:
        ...
        dtypes = descr['dtype']
        liste_files_filtered = [file for file in fs.ls(file_desc['minio_url']) if file_desc['file_format'] in file]
        create_table(file_desc['table_name'], query_create_table, force_import=force_import)
        
        for file in liste_files_filtered:
            try:
                ...
                if isinstance(e, FileNotFoundError):
                    logger.error(F"{e.__class__.__name__} - {file}")
                # errors are already in the logs
                import_errors.append(F"{e.__class__.__name__} - {file}")
                continue
